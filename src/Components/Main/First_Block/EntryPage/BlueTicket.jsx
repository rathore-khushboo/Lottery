...
const Tickets = () => {
  ...
  const handleAddTicket = () => {
    if (selectedNumbers.length < 4) {
      alert("Please select 4 numbers before adding another ticket.");
      return;
    }
  ...
  return (
    <div>
      <div className="flex justify-center items-center mt-10">
        <div className="relative w-96 h-96 bg-blue-500 rounded-lg shadow-lg p-4">
          <button
            className="absolute top-0 right-0 m-2 p-2 bg-white text-blue-500 border border-blue-500 rounded-full"
          ...
          {selectedNumbers.length > 0 && (
            <div className="flex justify-center mb-4">
              {selectedNumbers.map((number, index) => (
                <div
                  key={index}
                  className="bg-white text-blue-500 border border-blue-500 rounded-lg px-3 py-1 flex items-center justify-center mr-2"
                >
                  {number}
                </div>
              ))}
          ...
          <div className="mt-4 space-y-4">
            <div className="flex justify-between">
              {[1, 2, 3, 4, 5].map((number) => (
                <button
                  key={number}
                  className="w-16 h-16 bg-white text-blue-500 border border-blue-500 rounded-lg flex items-center justify-center"
          ...
          {additionalTickets.map((ticket, ticketIndex) => (
            <div key={ticketIndex} className="bg-white rounded-lg shadow-lg p-4 mb-4 max-w-xs">
              <div className="flex justify-between items-center mb-2">
                <span className="text-blue-500 font-bold">Ticket {ticketIndex + 1}</span>
                <button
              ...
              <div className="flex justify-center">
                {ticket.map((number, index) => (
                  <div
                    key={`${ticketIndex}-${index}`}
                    className="text-white bg-blue-600 border border-blue-500 rounded-lg px-3 py-1 flex items-center justify-center mr-2"
                  >
                    {number}
                  </div>
                ))}
